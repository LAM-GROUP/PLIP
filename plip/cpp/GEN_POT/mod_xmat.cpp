	...
	int N_max=25;
		...
		int N_selectedNB=0;
			...
			if (abs(coeffS[i_func])>1e-10)
			{
				...
				streamObj << coeffS[i_func];
				string strObj = streamObj.str();
				string command="python3 GenEAM.py  "+to_string(N_selectedNB)+"  "+to_string(i_type)+"  "+to_string(m)+"  "+to_string(list_a_[i_a])+"  "+to_string(list_b_[i_b])+"  "+strObj+"  "+to_string(Rcut)+"  "+to_string(i_nature_)+"  "+to_string(Rshort_tmp);
				cout<<command<<endl;
